...
function _mt:init()
  ...
  if not ver then
    return nil, "failed to retrieve HighGo server_version_num: " .. err
  end
  ...
  DELETE
   ...
   WHERE ctid IN (TABLE rows);]], table_name_escaped, column_name, "%s", column_name, table_name_escaped)
    ...
    return timer_every(self.config.ttl_cleanup_interval, function(premature)
      ...
      if not ok then
        log(WARN, "unable to fetch current timestamp from HighGo database (",
          err, ")")
        return
      end
          ...
          if not ok then
            if err then
              log(WARN, "unable to clean expired rows from table '",
                table_names[i], "' on HighGo database (",
                err, ")")
...
function _M.new(kong_config)
  local config = {
    host        = kong_config.highgo_host,
    port        = kong_config.highgo_port,
    timeout     = kong_config.highgo_timeout,
    user        = kong_config.highgo_user,
    password    = kong_config.highgo_password,
    database    = kong_config.highgo_database,
    schema      = kong_config.highgo_schema or "",
    ssl         = kong_config.highgo_ssl,
    ssl_verify  = kong_config.highgo_ssl_verify,
    cafile      = kong_config.lua_ssl_trusted_certificate_combined,
    sem_max     = kong_config.highgo_max_concurrent_queries or 0,
    sem_timeout = (kong_config.highgo_semaphore_timeout or 60000) / 1000,
    pool_size   = kong_config.highgo_pool_size,
    backlog     = kong_config.highgo_backlog,

    --- not used directly by pgmoon, but used internally in connector to set the keepalive timeout
    keepalive_timeout = kong_config.highgo_keepalive_timeout,
    --- non user-faced parameters
    ttl_cleanup_interval = kong_config._debug_highgo_ttl_cleanup_interval or 300,
  }

  local refs = kong_config["$refs"]
  if refs then
    local user_ref = refs.highgo_user
    local password_ref = refs.highgo_password
  ...
  if config.sem_max > 0 then
    ...
    if not sem then
      ngx.log(ngx.CRIT, "failed creating the HighGo connector semaphore: ",
        err)
    end
  end
  ...
  if not ngx.IS_CLI and kong_config.highgo_ro_host then
    ngx.log(ngx.DEBUG, "HighGo connector readonly connection enabled")

    local ro_override = {
      host        = kong_config.highgo_ro_host,
      port        = kong_config.highgo_ro_port,
      timeout     = kong_config.highgo_ro_timeout,
      user        = kong_config.highgo_ro_user,
      password    = kong_config.highgo_ro_password,
      database    = kong_config.highgo_ro_database,
      schema      = kong_config.highgo_ro_schema,
      ssl         = kong_config.highgo_ro_ssl,
      ssl_verify  = kong_config.highgo_ro_ssl_verify,
      cafile      = kong_config.lua_ssl_trusted_certificate_combined,
      sem_max     = kong_config.highgo_ro_max_concurrent_queries,
      sem_timeout = kong_config.highgo_ro_semaphore_timeout and
        (kong_config.highgo_ro_semaphore_timeout / 1000) or nil,
      pool_size   = kong_config.highgo_ro_pool_size,
      backlog     = kong_config.highgo_ro_backlog,

      --- not used directly by pgmoon, but used internally in connector to set the keepalive timeout
      keepalive_timeout = kong_config.highgo_ro_keepalive_timeout,
    }

    if refs then
      local ro_user_ref = refs.highgo_ro_user
      local ro_password_ref = refs.highgo_ro_password
