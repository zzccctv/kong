...
local function deserialize_row(self, row)
  if row then
    for field_name, field in self.schema:each_field() do
      ...
      elseif row[field_name] and field.type == 'record' then
        row[field_name] = cjson.decode(row[field_name])
      elseif field.type == "boolean" and row[field_name] then
        row[field_name] = row[field_name] == '1'
      elseif field.type == "integer" and row[field_name] then
        row[field_name] = tonumber(row[field_name])
      elseif field.type == "number" and row[field_name]  then
        row[field_name] = tonumber(row[field_name])
      end
    end
  end
...
local function escape_literal(connector, literal, field)
  ...
  if field then
    ...
    if field.type == "boolean" then
      return literal and 1 or 0
    end

    if type(literal) == "table" then
      if field.type == "array" or field.type == "set" then
        return connector:escape_literal(tableToString(literal))
...
function _mt:insert(entity, options)
  ...
  if res then
    local row = deserialize_row(self, res[1])
    if row then
...
function _M.new(connector, schema, errors)
  ...
  local where_expressions = {}
  for i = 1, #primary_key_escaped do
    insert(where_expressions, primary_key_escaped[i] .. " = " .. primary_key_placeholders[i])
  end
  where_expressions = concat(where_expressions, " and ")

  local where_update_expressions = {}
  for i = 1, #primary_key_escaped do
    insert(where_update_expressions, primary_key_escaped[i] .. " = " .. update_placeholders[i])
  end
  where_update_expressions = concat(where_update_expressions, " and ")
  ...
  if has_ttl then
    ...
    select_expressions = concat {
      select_expressions, ",",
      "EXTRACT(SECOND FROM ",
      ttl_escaped, " - CURRENT_TIMESTAMP)",
      " AS ", ttl_escaped
    }

    ttl_select_where = concat {
      "(", ttl_escaped, " IS NULL OR ", ttl_escaped, " >= CURRENT_TIMESTAMP)"
    }
  end

  local merge_insert = {}
  for i = 1, #insert_columns do
    insert(merge_insert, insert_expressions[i] .. " as " .. insert_columns[i])
  end
  merge_insert = concat(merge_insert, ",")
  ...
  add_statement("update", {
    ...
    code =  {
      ...
      where_clause(
        "    WHERE ", where_update_expressions,
        ttl_select_where,
        ws_id_select_where), ";",
      "SELECT ", select_expressions, " FROM ", table_name, "\n",
      where_clause(
        "    WHERE ", where_update_expressions,
        ttl_select_where,
        ws_id_select_where), "limit 1;",
    }
  })
  ...
  if #foreign_key_list > 0 then
    for foreign_entity_name, foreign_key in pairs(foreign_keys) do
      ...
      local fk_expressions = {}
      for i = 1, #fk_escaped do
        insert(fk_expressions, fk_escaped[i] .. " = " .. fk_placeholders[i])
      end
      fk_expressions = concat(fk_expressions, " and ")
