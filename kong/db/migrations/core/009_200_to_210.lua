...
local function highgo_ca_certificates_migration(connector)
  for ca_cert, err in connector:iterate("SELECT id, cert, cert_digest FROM ca_certificates") do
    if err then
      return nil, err
    end
    ...
    up = [[
        ...
        DO $$
          ...
          EXCEPTION WHEN undefined_column THEN
            -- Do nothing, accept existing state
        END$$;
    ]] .. ws_migration_up(operations.highgo.up),
    teardown = function(connector)
      local _, err = ws_migration_teardown(operations.highgo.teardown)(connector)
      if err then
        return nil, err
      end

      -- add `cert_digest` field for `ca_certificates` table
      _, err = highgo_ca_certificates_migration(connector)
      if err then
        return nil, err
      end
