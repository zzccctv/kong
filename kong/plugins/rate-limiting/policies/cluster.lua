    ...
    increment = function(connector, limits, identifier, current_timestamp, service_id, route_id, value)
      ...
      for _, period in ipairs(timestamp.timestamp_table_fields) do
        ...
        if limits[period] then
          len = len + 1
          buf[len] = fmt([[
            MERGE INTO ratelimiting_metrics AS t
            USING (select %s as "identifier", %s as "period", FROM_UNIXTIME(%s) as "period_date", %s as "service_id", %s as "route_id", %s as "value", CURRENT_TIMESTAMP(3)+%.1f/24/60/60 as "ttl") AS s
            ON (t."identifier" = s."identifier" and t."period" = s."period" and t."period_date" = s."period_date" and t."service_id" = s."service_id" and t."route_id" = s."route_id")
            WHEN MATCHED THEN
            UPDATE SET t."value" = t."value" + s."value"
            WHEN NOT MATCHED THEN
            INSERT ("identifier", "period", "period_date", "service_id", "route_id", "value", "ttl")
                  VALUES (s."identifier",s."period",s."period_date",s."service_id",s."route_id",s."value",s."ttl")
          ]],
            connector:escape_literal(identifier),
            connector:escape_literal(period),
            connector:escape_literal(tonumber(fmt("%.3f", floor(period_date / 1000)))),
            ...
            connector:escape_literal(route_id),
            connector:escape_literal(value),
            connector:escape_literal(EXPIRATION[period]))
        end
      end
