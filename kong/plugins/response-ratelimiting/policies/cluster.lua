  ...
  dm = {
    increment = function(connector, identifier, name, current_timestamp, service_id, route_id, value)
      ...
      for _, period in ipairs(timestamp.timestamp_table_fields) do
        ...
        len = len + 1
        buf[len] = fmt([[
            MERGE INTO response_ratelimiting_metrics AS t
            USING (select %s as "identifier", %s as "period", FROM_UNIXTIME(%s) as "period_date", %s as "service_id", %s as "route_id", %s as "value") AS s
            ...
            INSERT ("identifier", "period", "period_date", "service_id", "route_id", "value")
                  VALUES (s."identifier",s."period",s."period_date",s."service_id",s."route_id",s."value")
        ]],
          connector:escape_literal(identifier),
          connector:escape_literal(fmt("%s_%s", name, period)),
          connector:escape_literal(tonumber(fmt("%.3f", floor(period_date / 1000)))),
