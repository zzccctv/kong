...
INSERT INTO cluster_events(id, node_id, at, nbf, expire_at, channel, data)
 ...
 %s,
 %s,
 DATE_ADD(CURRENT_TIMESTAMP, INTERVAL %.1f SECOND),
 %s,
 %s)
 ]]
...
SELECT  UNIX_TIMESTAMP(CURRENT_TIMESTAMP) AS "now"
]]
...
function _M:insert(node_id, channel, at, data, delay)
  ...
  local nbf
  if delay then
    nbf = fmt("DATE_ADD(CURRENT_TIMESTAMP, INTERVAL %.1f SECOND)", delay)
  else
    nbf = "NULL"
  end
